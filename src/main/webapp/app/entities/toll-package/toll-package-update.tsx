...
import { getEntities as getTollSubscriptions } from 'app/entities/toll-subscription/toll-subscription.reducer';
...
export const TollPackageUpdate = () => {
  ...
  const tollSubscriptions = useAppSelector(state => state.tollSubscription.entities);
  ...
  const saveEntity = values => {
    ...
    const entity = {
      ...tollPackageEntity,
      ...values,
      tollSubscription: tollSubscriptions.find(it => it.id.toString() === values.tollSubscription.toString()),
    };
  ...
  const defaultValues = () =>
    isNew
      ...
      : {
          ...
          tollSubscription: tollPackageEntity?.tollSubscription?.id,
        };
        ...
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              />
              <ValidatedField
                id="toll-package-tollSubscription"
                name="tollSubscription"
                data-cy="tollSubscription"
                label={translate('rfidh2MysqlApp.tollPackage.tollSubscription')}
                type="select"
              >
                <option value="" key="0" />
                {tollSubscriptions
                  ? tollSubscriptions.map(otherEntity => (
