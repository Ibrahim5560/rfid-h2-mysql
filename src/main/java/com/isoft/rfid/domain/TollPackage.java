...
public class TollPackage implements Serializable {
    ...
    @OneToMany(mappedBy = "tollPackage")
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @JsonIgnoreProperties(value = { "tollPackage" }, allowSetters = true)
    private Set<TollSubscription> tollSubscriptions = new HashSet<>();
    ...
    public Set<TollSubscription> getTollSubscriptions() {
        return this.tollSubscriptions;
    }

    public void setTollSubscriptions(Set<TollSubscription> tollSubscriptions) {
        if (this.tollSubscriptions != null) {
            this.tollSubscriptions.forEach(i -> i.setTollPackage(null));
        }
        if (tollSubscriptions != null) {
            tollSubscriptions.forEach(i -> i.setTollPackage(this));
        }
        this.tollSubscriptions = tollSubscriptions;
    }

    public TollPackage tollSubscriptions(Set<TollSubscription> tollSubscriptions) {
        this.setTollSubscriptions(tollSubscriptions);
        return this;
    }

    public TollPackage addTollSubscription(TollSubscription tollSubscription) {
        this.tollSubscriptions.add(tollSubscription);
        tollSubscription.setTollPackage(this);
        return this;
    }

    public TollPackage removeTollSubscription(TollSubscription tollSubscription) {
        this.tollSubscriptions.remove(tollSubscription);
        tollSubscription.setTollPackage(null);
        return this;
    }
